...
For each sample name found in ``labels/``, the script loads the matching mesh
from ``meshes/<name>.obj`` and tries to load a signal from either::

    <input_dir>/signals/<name>.npy
    <input_dir>/<name>.npy

If no signal array is present, the z-coordinate of each vertex is used as a
fallback scalar field so a plot can still be produced.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("input_dir", help="Dataset root containing labels/, meshes/, and optional signals/.")
...
def load_signal(input_dir: Path, sample_name: str, mesh: trimesh.Trimesh) -> tuple[np.ndarray, str]:
    signal_candidates = [
        input_dir / "signals" / f"{sample_name}.npy",
        input_dir / f"{sample_name}.npy",
    ]

    for signal_path in signal_candidates:
    ...
    return np.asarray(mesh.vertices[:, 2], dtype=float), "vertex z-coordinate fallback"
...
def plot_sample(
    ...
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")
    ax.set_title(f"{title}: {sample_name}")
